...
export default class App extends Component {
  ...
  private onDownloadPathChanged() {
    this.updateConfig();
    electron.send("set-download-path", this.config.downloadPath);
  }

  private onImageKeydown(index: number, ev: KeyboardEvent) {
  ...
  private onKeydown({ key, ctrlKey }: KeyboardEvent) {
    switch (key) {
      case "F12": {
        return electron.send("toggle-dev-tools");
      }
      case "F5": {
        return location.reload();
      }
      case "R": {
        return ctrlKey && location.reload();
      }
      case "Escape": {
  ...
  private pageNext() {
    this.pageSet(this.state.pageIndex + 1);
  }

  private pagePrev() {
    this.pageSet(this.state.pageIndex - 1, this.rows * this.cols - 1);
  }

  private pageSet(pageIndex: number, focusIndex: number | null = null) {
    if (
      pageIndex < 0 ||
  ...
  private async reset(...whiteListed: string[]) {
